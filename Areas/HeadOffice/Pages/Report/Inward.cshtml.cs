...
{
    public class InwardModel : PageModel
    {
        ApplicationDbContext _context;
        public InwardModel(ApplicationDbContext context)
        {
            _context = context;

        }

        public List<SpInwardReport> SpInwardReport { get; set; }
        ...
        public async Task<IActionResult> OnGet(DateTime startdate, DateTime enddate, string submit)
        {
            if (string.IsNullOrEmpty(submit))
            {
                ...
                SpInwardReport = await _context.SpInwardReport.FromSqlRaw("SpInwardReport @Startdate,@Enddate,@WareHouse", startdates, enddates, warehouses).ToListAsync();
